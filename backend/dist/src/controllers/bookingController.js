...
import stripeAPI from "../payment/stripe.js";
/**
 * Create a Booking.
 *
 * @export
 ...
 * @returns {void}
 */
export const notify = async (renter, bookingId, user, notificationMessage) => {
 ...
 * @returns {unknown}
 */
export const confirm = async (user, booking, payLater) => {
  const {
    language
  } = user;
  ...
  const property = await Property.findById(booking.property).populate('agency');
  if (!property) {
  ...
  const mailOptions = {
    ...
    html: `<p>${i18n.t('HELLO')}${user.fullName},<br><br>
        ...
        ${i18n.t('BOOKING_CONFIRMED_PART3')}${property.agency.fullName}${i18n.t('BOOKING_CONFIRMED_PART4')}${i18n.t('BOOKING_CONFIRMED_PART5')}` + `${from} ${i18n.t('BOOKING_CONFIRMED_PART6')}` + `${property.name}${i18n.t('BOOKING_CONFIRMED_PART7')}` + `${property.address && `<br><br>${i18n.t('BOOKING_CONFIRMED_ADDRESS')}${property.address}` || ''}` + `${property.latitude && property.longitude && `<br><br>${i18n.t('BOOKING_CONFIRMED_MAP_PART1')}<a href='https://maps.google.com/?q=${property.latitude},${property.longitude}'>${i18n.t('BOOKING_CONFIRMED_MAP_PART2')}</a>` || ''}` + `<br><br>${i18n.t('BOOKING_CONFIRMED_PART8')}<br><br>` + `${i18n.t('BOOKING_CONFIRMED_PART9')}${property.agency.fullName}${i18n.t('BOOKING_CONFIRMED_PART10')}${i18n.t('BOOKING_CONFIRMED_PART11')}` + `${to} ${i18n.t('BOOKING_CONFIRMED_PART12')}` + `<br><br>${i18n.t('BOOKING_CONFIRMED_PART13')}<br><br>${i18n.t('BOOKING_CONFIRMED_PART14')}${env.FRONTEND_HOST}<br><br>
        ${i18n.t('REGARDS')}<br></p>`
  };
  await mailHelper.sendMail(mailOptions);
  return true;
 ...
 * @returns {*}
 */
const notifyRenter = async booking => {
...
export const update = async (req, res) => {
  try {
    ...
    if (booking) {
      ...
      booking.agency = new mongoose.Types.ObjectId(agency);
      booking.location = new mongoose.Types.ObjectId(location);
      booking.property = new mongoose.Types.ObjectId(property);
      booking.renter = new mongoose.Types.ObjectId(renter);
      booking.from = from;
      booking.to = to;
...
export const deleteTempBooking = async (req, res) => {
  ...
  try {
    ...
    logger.error(`[booking.deleteTempBooking] ${i18n.t('ERROR')} ${JSON.stringify({
      bookingId,
      sessionId
    })}`, err);
    res.status(400).send(i18n.t('ERROR') + err);
...
export const getBooking = async (req, res) => {
  ...
  try {
    const booking = await Booking.findById(id).populate('agency').populate({
      path: 'location',
      populate: {
        path: 'values',
        model: 'LocationValue'
...
export const getBookings = async (req, res) => {
  try {
    ...
    let agencies = body.agencies.map(id => new mongoose.Types.ObjectId(id));
    const {
      statuses,
      // user,
      property
    ...
    if (sessionUser.type === movininTypes.UserType.User) {
      $match.$and.push({
        'renter._id': {
          $eq: new mongoose.Types.ObjectId(sessionUser._id)
        }
      });
    }
    // end of security check
    ...
    if (property) {
      $match.$and.push({
        'property._id': {
          $eq: new mongoose.Types.ObjectId(property)
        }
      });
    }
    if (location) {
      $match.$and.push({
        'location._id': {
          $eq: new mongoose.Types.ObjectId(location)
        }
      });
    }
    if (from) {
...
export const cancelBooking = async (req, res) => {
  ...
  try {
    ...
    }).populate('agency').populate('renter');
