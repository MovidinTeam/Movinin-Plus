...
export const getLocation = async (req, res) => {
  ...
  try {
    ...
    if (location) {
      ...
      const name = location.values.filter(value => value.language === language)[0].value;
      if (location.country) {
        const countryName = location.country.values.filter(value => value.language === req.params.language)[0].value;
        location.country.name = countryName;
      }
      let parentLocation;
      if (location.parentLocation) {
        const parentLocationName = location.parentLocation.values.filter(value => value.language === language)[0].value;
        parentLocation = {
 ...
 * @returns {unknown}
 */
export const getLocationsWithPosition = async (req, res) => {
  try {
    const {
      language
    } = req.params;
...
export const checkLocation = async (req, res) => {
  ...
  try {
    ...
    const childLocationsCount = await Location.find({
      parentLocation: _id
    }).limit(1).countDocuments();
 ...
 * @returns {unknown}
 */
export const getLocationId = async (req, res) => {
  const {
    name,
    language
  } = req.params;
