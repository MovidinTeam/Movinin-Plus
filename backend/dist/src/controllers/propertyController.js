...
export const update = async (req, res) => {
  ...
  try {
    ...
    if (property) {
      ...
      property.rentalTerm = rentalTerm;
 ...
 * @returns {unknown}
 */
export const checkProperty = async (req, res) => {
  const {
    id
  } = req.params;
  try {
...
export const getProperty = async (req, res) => {
  ...
  try {
    const property = await Property.findById(id).populate('agency').populate({
      path: 'location',
      populate: {
        path: 'values',
        model: 'LocationValue'
 ...
 * @returns {unknown}
 */
export const getBookingProperties = async (req, res) => {
  try {
    const {
      body
    } = req;
 ...
 * @returns {unknown}
 */
export const getFrontendProperties = async (req, res) => {
  try {
    const {
      body
    } = req;
          ...
          $match: {
            $expr: {
              $and: [{
                ...
                $in: ['$status', [movininTypes.BookingStatus.Paid, movininTypes.BookingStatus.Reserved, movininTypes.BookingStatus.Deposit]]
              }]
            }
          }
        }],
