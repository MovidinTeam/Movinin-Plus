...
export const decryptJWT = async input => {
  const {
    payload
  } = await jose.jwtVerify(input, jwtSecret, {
    algorithms: [jwtAlg]
  });
  return payload;
};
 ...
 * @returns {boolean}
 */
export const isAdmin = req => !!req.headers.origin && helper.trimEnd(req.headers.origin, '/') === helper.trimEnd(env.ADMIN_HOST, '/');
/**
 ...
 * APPLE: Always a JWT
 */
export async function verifyAppleToken(token, email) {
  try {
    // Define all valid identifiers
    const validAudiences = [env.APPLE_CLIENT_ID_WEB, env.APPLE_CLIENT_ID_MOBILE];
    const {
      payload
    } = await jose.jwtVerify(token, APPLE_JWKS, {
      issuer: 'https://appleid.apple.com',
      audience: validAudiences,
      // Jose allows an array here
      clockTolerance: '2m'
    });
 ...
 */
export async function verifyGoogleToken(token, email) {
  const allowedClientIds = [env.GOOGLE_CLIENT_ID, env.GOOGLE_MOBILE_CLIENT_ID];
 ...
 */
export async function verifyFacebookToken(token, email) {
