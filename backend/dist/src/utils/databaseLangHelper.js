 ...
 */
const syncLanguageValues = async (collection, label) => {
  try {
    // Load all documents with populated 'values' field
    const docs = await collection.find({}).populate({
      path: 'values',
      model: 'LocationValue'
    });
    const newValues = [];
    ...
    for (const doc of docs) {
      ...
      if (!en) {
        logger.warn(`English value missing for ${label} document:`, doc._id.toString());
        continue;
      }
    ...
    const cursor = LocationValue.find({
      language: {
        $nin: env.LANGUAGES
      }
    }, {
    ...
    for await (const obsoleteVal of cursor) {
      ...
      if (obsoleteIdsBatch.length >= env.BATCH_SIZE) {
        await Promise.all([collection.updateMany({
          values: {
            $in: obsoleteIdsBatch
          }
        }, {
