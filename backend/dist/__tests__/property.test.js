...
describe('POST /api/create-property', () => {
  it('should create a property', async () => {
    ...
    let res = await request(app).post('/api/create-property').set(env.X_ACCESS_TOKEN, token).send(payload);
    ...
    res = await request(app).post('/api/create-property').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    const deleteRes = await Property.deleteOne({
      _id: res.body._id
    });
    ...
    res = await request(app).post('/api/create-property').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    res = await request(app).post('/api/create-property').set(env.X_ACCESS_TOKEN, token).send({
      image: 'image.jpg'
    });
    expect(res.statusCode).toBe(400);
...
describe('PUT /api/update-property', () => {
  it('should update a property', async () => {
    ...
    let res = await request(app).put('/api/update-property').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    payload.image = '';
    payload.images = undefined;
    res = await request(app).put('/api/update-property').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    property = res.body;
    expect(property.image).toBeDefined();
    ...
    res = await request(app).put('/api/update-property').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    res = await request(app).put('/api/update-property').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-property-image/:id/:image', () => {
  it('should delete an additional property image', async () => {
    ...
    let res = await request(app).post(`/api/delete-property-image/${PROPERTY_ID}/${additionalImageName}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    property = await Property.findById(PROPERTY_ID);
    ...
    await property?.save();
    res = await request(app).post(`/api/delete-property-image/${PROPERTY_ID}/${image}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    res = await request(app).post(`/api/delete-property-image/${testHelper.GetRandromObjectIdAsString()}/${additionalImageName}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).post(`/api/delete-property-image/${PROPERTY_ID}/unknown.jpg`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).post(`/api/delete-property-image/0/${additionalImageName}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/upload-property-image', () => {
  it('should upload a property image', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).post('/api/upload-property-image').set(env.X_ACCESS_TOKEN, token).attach('image', MAIN_IMAGE1_PATH);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/upload-property-image').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-temp-property-image/:image', () => {
  it('should delete a temporary property image', async () => {
    ...
    let res = await request(app).post(`/api/delete-temp-property-image/${MAIN_IMAGE1}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
...
describe('GET /api/property/:id/:language', () => {
  it('should return a property', async () => {
    let res = await request(app).get(`/api/property/${PROPERTY_ID}/${testHelper.LANGUAGE}`);
    expect(res.statusCode).toBe(200);
    expect(res.body.name).toBe('Beautiful Townhouse in Detroit');
    res = await request(app).get(`/api/property/${testHelper.GetRandromObjectIdAsString()}/${testHelper.LANGUAGE}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/property/0/${testHelper.LANGUAGE}`);
    expect(res.statusCode).toBe(400);
  });
});
describe('POST /api/properties/:page/:size', () => {
  it('should return properties', async () => {
    ...
    let res = await request(app).post(`/api/properties/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBeGreaterThan(0);
    res = await request(app).post(`/api/properties/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    payload.availability = [movininTypes.Availablity.Available];
    res = await request(app).post(`/api/properties/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBeGreaterThan(0);
...
describe('POST /api/booking-properties/:page/:size', () => {
  it('should return booking properties', async () => {
    ...
    let res = await request(app).post(`/api/booking-properties/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body.length).toBeGreaterThan(0);
    res = await request(app).post(`/api/booking-properties/unknown/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/frontend-properties/:page/:size', () => {
  it('should return frontend properties', async () => {
    ...
    let res = await request(app).post(`/api/frontend-properties/${testHelper.PAGE}/${testHelper.SIZE}`).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBeGreaterThan(0);
    payload.types = undefined;
    payload.rentalTerms = undefined;
    res = await request(app).post(`/api/frontend-properties/${testHelper.PAGE}/${testHelper.SIZE}`).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBe(0);
    ...
    res = await request(app).post(`/api/frontend-properties/${testHelper.PAGE}/${testHelper.SIZE}`);
    expect(res.statusCode).toBe(400);
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      res = await request(newApp).post(`/api/frontend-properties/${testHelper.PAGE}/${testHelper.SIZE}`).send(payload);
      expect(res.statusCode).toBe(200);
...
describe('GET /api/check-property/:id', () => {
  it('should check a property', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/check-property/${PROPERTY_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    ...
    await booking.save();
    res = await request(app).get(`/api/check-property/${PROPERTY_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    await Booking.deleteOne({
      _id: booking._id
    });
    res = await request(app).get('/api/check-property/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    res = await request(app).get('/api/check-property/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
...
describe('DELETE /api/delete-property/:id', () => {
  it('should delete a property', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).delete(`/api/delete-property/${PROPERTY_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    ...
    await property.save();
    res = await request(app).delete(`/api/delete-property/${property._id.toString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).delete(`/api/delete-property/${property._id.toString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    res = await request(app).delete(`/api/delete-property/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).delete('/api/delete-property/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    res = await request(app).delete('/api/delete-property/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
