...
import Booking, { BOOKING_EXPIRE_AT_INDEX_NAME } from "../src/models/Booking.js";
import User, { USER_EXPIRE_AT_INDEX_NAME } from "../src/models/User.js";
import Token, { TOKEN_EXPIRE_AT_INDEX_NAME } from "../src/models/Token.js";
...
const createBookingIndex = async expireAfterSeconds => {
...
const createUserIndex = async expireAfterSeconds => {
  await User.collection.createIndex({
    expireAt: 1
  }, {
    name: USER_EXPIRE_AT_INDEX_NAME,
    expireAfterSeconds,
    background: true
  });
};
const createTokenIndex = async expireAfterSeconds => {
...
describe('Test database initialization', () => {
  it('should test database initialization', async () => {
    ...
    expect(res).toBeTruthy();
    await LocationValue.collection.dropIndex(indexName);
    await LocationValue.collection.createIndex({
      ['value']: 'text'
    }, opts);
    res = await databaseHelper.initialize(false);
    expect(res).toBeTruthy();
