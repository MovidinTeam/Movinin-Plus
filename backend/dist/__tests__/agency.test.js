...
describe('POST /api/validate-agency', () => {
  it('should validate a agency', async () => {
    ...
    let res = await request(app).post('/api/validate-agency').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(204);
    payload.fullName = testHelper.getAgencyName();
    res = await request(app).post('/api/validate-agency').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    res = await request(app).post('/api/validate-agency').set(env.X_ACCESS_TOKEN, token);
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      res = await request(newApp).post('/api/validate-agency').set(env.X_ACCESS_TOKEN, newToken).send(payload);
      expect(res.statusCode).toBe(400);
...
describe('PUT /api/update-agency', () => {
  it('should update an agency', async () => {
    ...
    let res = await request(app).put('/api/update-agency').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).put('/api/update-agency').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(204);
    // test failure (invalid id
    payload._id = '0';
    res = await request(app).put('/api/update-agency').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    res = await request(app).put('/api/update-agency').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/agency/:id', () => {
  it('should get a agency', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/agency/${AGENCY1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    expect(res.body.fullName).toBe(AGENCY1_NAME);
    res = await request(app).get(`/api/agency/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).get('/api/agency/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/agencies/:page/:size', () => {
  it('should get agencies', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/agencies/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBeGreaterThan(1);
    res = await request(app).get(`/api/agencies/unknown/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    res = await request(app).get(`/api/agencies/${testHelper.PAGE}/${testHelper.SIZE}?s=${nanoid()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
...
describe('GET /api/all-agencies', () => {
  it('should get all agencies', async () => {
    let res = await request(app).get('/api/all-agencies');
    expect(res.statusCode).toBe(200);
    expect(res.body.length).toBeGreaterThan(1);
    await databaseHelper.close();
    res = await request(app).get('/api/all-agencies');
    expect(res.statusCode).toBe(400);
...
describe('DELETE /api/delete-agency/:id', () => {
  it('should delete a agency', async () => {
    ...
    await property.save();
    let res = await request(app).delete(`/api/delete-agency/${agencyId}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).delete(`/api/delete-agency/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).delete('/api/delete-agency/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    ...
    expect(agency).not.toBeNull();
    await agency?.save();
    res = await request(app).delete(`/api/delete-agency/${agencyId}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    agency = await User.findById(agencyId);
    expect(agency).toBeNull();
    agencyName = testHelper.getAgencyName();
