...
describe('POST /api/create-paypal-order', () => {
  it('should create paypal order', async () => {
    ...
    expect(paypal.createOrder).toHaveBeenCalledWith('booking123', 100, 'USD', 'Test Name', 'Test Description', 'US');
    ...
    let res = await request(app).post('/api/create-paypal-order').send(payload);
    ...
    res = await request(app).post('/api/create-paypal-order').send(payload);
...
describe('POST /api/check-paypal-order/:bookingId/:orderId', () => {
  it('should check paypal order', async () => {
    ...
    try {
      ...
      let paypal = await import("../src/payment/paypal.js");
      await expect(paypal.getOrder('123')).resolves.toStrictEqual({
        status: 'COMPLETED'
      });
      expect(paypal.getOrder).toHaveBeenCalledWith('123');
      let res = await request(app).post(`/api/check-paypal-order/${booking._id.toString()}/${orderId}`);
      expect(res.statusCode).toBe(200);
      ...
      await booking.save();
      res = await request(app).post(`/api/check-paypal-order/${booking._id.toString()}/${orderId}`);
      expect(res.statusCode).toBe(200);
      await testHelper.deleteNotifications(booking._id.toString());
      ...
      res = await request(app).post(`/api/check-paypal-order/${booking._id.toString()}/${testHelper.GetRandromObjectIdAsString()}`);
      expect(res.statusCode).toBe(204);
      ...
      expect(paypal.getOrder).toHaveBeenCalledWith('123');
      res = await request(app).post(`/api/check-paypal-order/${booking2._id.toString()}/${orderId}`);
      expect(res.statusCode).toBe(400);
      ...
      expect(paypal.getOrder).toHaveBeenCalledWith('123');
      // property missing
      res = await request(app).post(`/api/check-paypal-order/${booking3._id.toString()}/${orderId}`);
      expect(res.statusCode).toBe(400);
      // agency missing
    ...
    let res = await request(app).post(`/api/check-paypal-order/${testHelper.GetRandromObjectIdAsString()}/${testHelper.GetRandromObjectIdAsString()}`);
    expect(res.statusCode).toBe(204);
    // test failure (lost db connection)
    try {
      databaseHelper.close();
      res = await request(app).post(`/api/check-paypal-order/${testHelper.GetRandromObjectIdAsString()}/${testHelper.GetRandromObjectIdAsString()}`);
      expect(res.statusCode).toBe(400);
    } catch (err) {
      console.error(err);
    } finally {
