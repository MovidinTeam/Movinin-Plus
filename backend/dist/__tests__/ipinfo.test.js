...
describe('GET /api/country-code', () => {
  it('should get country from ip', async () => {
    ...
    let res = await request(app).get('/api/country-code');
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('US');
    // test with x-forwarded-for
    res = await request(app).get('/api/country-code').set('x-forwarded-for', ip);
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('FR');
    // test with x-forwarded-for IPv6-mapped IPv4 address
    res = await request(app).get('/api/country-code').set('x-forwarded-for', '::ffff:51.91.60.241');
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('FR');
    // test wrong ip address
    res = await request(app).get('/api/country-code').set('x-forwarded-for', 'wrong ip address');
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe('US');
