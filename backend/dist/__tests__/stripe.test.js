...
describe('POST /api/create-checkout-session', () => {
  it('should create checkout session', async () => {
    ...
    let res = await request(app).post('/api/create-checkout-session').send(payload);
    ...
    await jest.unstable_mockModule('../src/payment/stripe.js', () => ({
      default: {
        customers: {
          list: jest.fn(() => Promise.reject(new Error('Stripe error'))),
          create: jest.fn(() => Promise.resolve({
            id: 'cus_new_456',
            email: 'new@example.com',
            name: 'John Doe'
...
describe('POST /api/check-checkout-session/:sessionId', () => {
  it('should check checkout session', async () => {
    ...
    try {
      ...
      let stripeAPI = (await import("../src/payment/stripe.js")).default;
      await expect(stripeAPI.checkout.sessions.retrieve('123')).resolves.toStrictEqual({
        payment_status: 'paid'
      });
      ...
      res = await request(app).post(`/api/check-checkout-session/${nanoid()}`);
      expect(res.statusCode).toBe(204);
      ...
      expect(stripeAPI.checkout.sessions.retrieve).toHaveBeenCalledWith('123');
      res = await request(app).post(`/api/check-checkout-session/${sessionId2}`);
      expect(res.statusCode).toBe(400);
      ...
      expect(stripeAPI.checkout.sessions.retrieve).toHaveBeenCalledWith('123');
      // property missing
      res = await request(app).post(`/api/check-checkout-session/${sessionId3}`);
      expect(res.statusCode).toBe(400);
      // agency missing
...
describe('POST /api/create-payment-intent', () => {
  it('should create payment intents', async () => {
    ...
    let res = await request(app).post('/api/create-payment-intent').send(payload);
    expect(res.statusCode).toBe(200);
