...
import * as databaseHelper from "../src/utils/databaseHelper.js";
...
describe('POST /api/sign-up', () => {
  it('should create a user', async () => {
    ...
    let res = await request(app).post('/api/sign-up').send(payload);
    ...
    res = await request(app).post('/api/sign-up').send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/sign-up');
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      res = await request(newApp).post('/api/sign-up').send(payload);
      expect(res.statusCode).toBe(200);
      user = await User.findOne({
        email: payload.email
      });
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      const deleteOneMock = jest.spyOn(User.prototype, 'deleteOne').mockImplementation(() => Promise.reject(new Error('deleteOne failed')));
      const payload = {
        fullName: 'user',
        email,
        language: testHelper.LANGUAGE,
...
describe.skip('POST /api/admin-sign-up', () => {
  it('should create an admin user', async () => {
    ...
    const res = await request(app).post('/api/admin-sign-up').send(payload);
    expect(res.statusCode).toBe(200);
...
describe('POST /api/create-user', () => {
  it('should create a user', async () => {
    ...
    let res = await request(app).post('/api/create-user').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    let user = await User.findOne({
      email: USER2_EMAIL
    });
    ...
    res = await request(app).post('/api/create-user').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    user = await User.findOne({
      email: payload.email
    });
    ...
    res = await request(app).post('/api/create-user').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    res = await request(app).post('/api/create-user').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    email = testHelper.GetRandomEmail();
    payload.email = email;
...
describe('GET /api/check-token/:type/:userId/:email/:token', () => {
  it("should check user's token", async () => {
    ...
    let res = await request(app).get(`/api/check-token/${movininTypes.AppType.Frontend}/${USER1_ID}/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(200);
    res = await request(app).get(`/api/check-token/${movininTypes.AppType.Admin}/${USER1_ID}/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/check-token/${movininTypes.AppType.Frontend}/${USER2_ID}/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/check-token/${movininTypes.AppType.Frontend}/${testHelper.GetRandromObjectIdAsString()}/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/check-token/${movininTypes.AppType.Frontend}/${USER1_ID}/${USER1_EMAIL}/${nanoid()}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/check-token/${movininTypes.AppType.Frontend}/0/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(400);
  });
});
describe('POST /api/activate', () => {
  it("should activate user's account", async () => {
    ...
    let res = await request(app).post('/api/activate').send(payload);
    expect(res.statusCode).toBe(200);
    const user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    ...
    res = await request(app).post('/api/activate').send(payload);
    expect(res.statusCode).toBe(204);
    ...
    res = await request(app).post('/api/activate').send(payload);
    expect(res.statusCode).toBe(400);
    res = await request(app).post('/api/activate');
...
describe('GET /api/confirm-email/:email/:token', () => {
  it('should send confirmation email', async () => {
    ...
    const token = userToken?.token;
    expect(token?.length).toBeGreaterThan(1);
    let res = await request(app).get(`/api/confirm-email/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(200);
    user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    expect(user?.verified).toBeTruthy();
    res = await request(app).get(`/api/confirm-email/${USER1_EMAIL}/${token}`);
    expect(res.statusCode).toBe(200);
    res = await request(app).get(`/api/confirm-email/${testHelper.GetRandomEmail()}/${token}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/confirm-email/${USER1_EMAIL}/${nanoid()}`);
    expect(res.statusCode).toBe(400);
    res = await request(app).get(`/api/confirm-email/unknown/${nanoid()}`);
    expect(res.statusCode).toBe(400);
  });
});
describe('POST /api/resend/:type/:email/:reset', () => {
  it('should resend validation email', async () => {
    ...
    await user.save();
    let reset = true;
    let res = await request(app).post(`/api/resend/${movininTypes.AppType.Frontend}/${USER1_EMAIL}/${reset}`);
    expect(res.statusCode).toBe(200);
    reset = false;
    ...
    expect(user).not.toBeNull();
    expect(user?.active).toBeFalsy();
    res = await request(app).post(`/api/resend/${movininTypes.AppType.Admin}/${USER1_EMAIL}/${reset}`);
    expect(res.statusCode).toBe(403);
    res = await request(app).post(`/api/resend/${movininTypes.AppType.Frontend}/${testHelper.GetRandomEmail()}/${reset}`);
    expect(res.statusCode).toBe(204);
    res = await request(app).post(`/api/resend/${movininTypes.AppType.Frontend}/unknown/${reset}`);
    expect(res.statusCode).toBe(400);
  });
});
describe('POST /api/resend-link', () => {
  it('should resend activation link', async () => {
    ...
    let res = await request(app).post('/api/resend-link').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    const user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    user.verified = true;
    await user?.save();
    res = await request(app).post('/api/resend-link').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    payload.email = testHelper.GetRandomEmail();
    res = await request(app).post('/api/resend-link').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
...
describe('DELETE /api/delete-tokens/:userId', () => {
  it("should delete user's tokens", async () => {
    ...
    let res = await request(app).delete(`/api/delete-tokens/${USER1_ID}`);
    expect(res.statusCode).toBe(200);
    userTokens = await Token.find({
      user: USER1_ID
    });
    expect(userTokens.length).toBe(0);
    res = await request(app).delete(`/api/delete-tokens/${USER1_ID}`);
    expect(res.statusCode).toBe(400);
    res = await request(app).delete('/api/delete-tokens/0');
    expect(res.statusCode).toBe(400);
  });
});
describe('POST /api/sign-in/:type', () => {
  it('should sign in', async () => {
    ...
    let res = await request(app).post(`/api/sign-in/${movininTypes.AppType.Frontend}`).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post(`/api/sign-in/${movininTypes.AppType.Frontend}`).send(payload);
    expect(res.statusCode).toBe(204);
    payload.password = USER1_PASSWORD;
    res = await request(app).post(`/api/sign-in/${movininTypes.AppType.Admin}`).send(payload);
    expect(res.statusCode).toBe(204);
...
describe('POST /api/social-sign-in/:type', () => {
  it('should sign in', async () => {
    ...
    let res = await request(app).post('/api/social-sign-in').send(payload);
    expect(res.statusCode).toBe(400);
    // test failure (facebook)
    payload.socialSignInType = movininTypes.SocialSignInType.Facebook;
    res = await request(app).post('/api/social-sign-in').send(payload);
    expect(res.statusCode).toBe(400);
    // test failure (apple)
    ...
    await jest.isolateModulesAsync(async () => {
      ...
      // test success (mobile)
      payload.mobile = true;
      res = await request(newApp).post('/api/social-sign-in').send(payload);
      expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/social-sign-in');
    expect(res.statusCode).toBe(400);
...
describe('POST /api/sign-out', () => {
  it('should sign out', async () => {
    const res = await request(app).post('/api/sign-out').set('Cookie', [`${env.X_ACCESS_TOKEN}=${nanoid()};`]);
    expect(res.statusCode).toBe(200);
    const cookies = res.headers['set-cookie'];
...
describe('POST /api/create-push-token/:userId/:token', () => {
  it('should create push token', async () => {
    ...
    let res = await request(app).post(`/api/create-push-token/${USER1_ID}/${pushToken}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    let pushNotifiation = await PushToken.findOne({
      user: USER1_ID,
      token: pushToken
    });
    expect(pushNotifiation).not.toBeNull();
    pushToken = nanoid();
    res = await request(app).post(`/api/create-push-token/${USER1_ID}/${pushToken}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    pushNotifiation = await PushToken.findOne({
      user: USER1_ID,
      token: pushToken
    });
    expect(pushNotifiation).toBeNull();
    res = await request(app).post(`/api/create-push-token/0/${pushToken}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/push-token/:userId', () => {
  it('should get push token', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/push-token/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    expect(res.body.length).toBeGreaterThan(1);
    res = await request(app).get(`/api/push-token/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).get('/api/push-token/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-push-token/:userId', () => {
  it('should delete push token', async () => {
    const token = await testHelper.signinAsAdmin();
    let pushNotifiations = await PushToken.find({
      user: USER1_ID
    });
    expect(pushNotifiations.length).toBeGreaterThan(0);
    let res = await request(app).post(`/api/delete-push-token/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    pushNotifiations = await PushToken.find({
      user: USER1_ID
    });
    expect(pushNotifiations.length).toBe(0);
    res = await request(app).post('/api/delete-push-token/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/validate-email', () => {
  it('should validate email', async () => {
    ...
    let res = await request(app).post('/api/validate-email').send(payload);
    expect(res.statusCode).toBe(204);
    payload.email = testHelper.GetRandomEmail();
    res = await request(app).post('/api/validate-email').send(payload);
    expect(res.statusCode).toBe(200);
...
describe('POST /api/validate-access-token', () => {
  it('should validate access token', async () => {
    ...
    // test success
    let res = await request(app).post('/api/validate-access-token').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    // test failure (unauthorized)
    res = await request(app).post('/api/validate-access-token').set(env.X_ACCESS_TOKEN, nanoid());
    expect(res.statusCode).toBe(401);
    // test failure (user not found)
    const payload = {
      id: testHelper.GetRandromObjectIdAsString()
    };
    token = await authHelper.encryptJWT(payload, true);
    res = await request(app).post('/api/validate-access-token').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(401);
    // test failure (forbidden)
    await testHelper.signout(token);
    res = await request(app).post('/api/validate-access-token');
    expect(res.statusCode).toBe(403);
  });
});
describe('POST /api/update-user', () => {
  it('should update user', async () => {
    ...
    let res = await request(app).post('/api/update-user').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    let user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    ...
    res = await request(app).post('/api/update-user').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    expect(user?.type).toBe(movininTypes.UserType.Agency);
...
describe('POST /api/update-email-notifications', () => {
  it('should update email notifications setting', async () => {
    ...
    let res = await request(app).post('/api/update-email-notifications').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    expect(user?.enableEmailNotifications).toBeTruthy();
    payload._id = testHelper.GetRandromObjectIdAsString();
    res = await request(app).post('/api/update-email-notifications').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(204);
    payload._id = '0';
    res = await request(app).post('/api/update-email-notifications').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
...
describe('POST /api/update-language', () => {
  it("should update user's language", async () => {
    ...
    let res = await request(app).post('/api/update-language').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    user = await User.findById(USER1_ID);
    expect(user).not.toBeNull();
    expect(user?.language).toBe(payload.language);
    payload.id = testHelper.GetRandromObjectIdAsString();
    res = await request(app).post('/api/update-language').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(204);
    payload.id = '0';
    res = await request(app).post('/api/update-language').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
...
describe('GET /api/user/:id', () => {
  it('should get a user', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/user/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    expect(res.body.email).toBe(USER1_EMAIL);
    res = await request(app).get(`/api/user/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).get('/api/user/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/create-avatar', () => {
  it("should create user's avatar", async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).post('/api/create-avatar').set(env.X_ACCESS_TOKEN, token).attach('image', AVATAR1_PATH);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/create-avatar').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/update-avatar/:userId', () => {
  it("should update user's avatar", async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).post(`/api/update-avatar/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token).attach('image', AVATAR2_PATH);
    expect(res.statusCode).toBe(200);
    const filename = res.body;
    ...
    await user?.save();
    res = await request(app).post(`/api/update-avatar/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token).attach('image', AVATAR2_PATH);
    expect(res.statusCode).toBe(200);
    ...
    avatarExists = await helper.pathExists(path.resolve(env.CDN_USERS, filename));
    expect(avatarExists).toBeTruthy();
    res = await request(app).post(`/api/update-avatar/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    res = await request(app).post(`/api/update-avatar/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token).attach('image', AVATAR2_PATH);
    expect(res.statusCode).toBe(204);
    res = await request(app).post('/api/update-avatar/0').set(env.X_ACCESS_TOKEN, token).attach('image', AVATAR2_PATH);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-avatar/:userId', () => {
  it("should delete user's avatar", async () => {
    ...
    expect(user?.avatar).toBeDefined();
    expect(user?.avatar).not.toBeNull();
    const filePath = path.join(env.CDN_USERS, user?.avatar);
    let avatarExists = await helper.pathExists(filePath);
    expect(avatarExists).toBeTruthy();
    let res = await request(app).post(`/api/delete-avatar/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    ...
    user.avatar = undefined;
    await user?.save();
    res = await request(app).post(`/api/delete-avatar/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    user.avatar = `${nanoid()}.jpg`;
    await user?.save();
    res = await request(app).post(`/api/delete-avatar/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    res = await request(app).post(`/api/delete-avatar/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).post('/api/delete-avatar/0').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-temp-avatar/:avatar', () => {
  it('should delete temporary avatar', async () => {
    ...
    let res = await request(app).post(`/api/delete-temp-avatar/${AVATAR1}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    const tempImageExists = await helper.pathExists(tempAvatar);
    expect(tempImageExists).toBeFalsy();
    res = await request(app).post('/api/delete-temp-avatar/unknown.jpg').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/change-password', () => {
  it('should change password', async () => {
    ...
    let res = await request(app).post('/api/change-password').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    payload.password = newPassword;
    payload.newPassword = USER1_PASSWORD;
    payload.strict = false;
    res = await request(app).post('/api/change-password').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    payload.strict = true;
    payload.password = '';
    res = await request(app).post('/api/change-password').set(env.X_ACCESS_TOKEN, token).send(payload);
...
describe('GET /api/check-password/:id/:password', () => {
  it('should check password', async () => {
    const token = await testHelper.signinAsAdmin();
    // good password
    let res = await request(app).get(`/api/check-password/${USER1_ID}/${encodeURIComponent(USER1_PASSWORD)}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    // wrong password
    res = await request(app).get(`/api/check-password/${USER1_ID}/wrong-password`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    // user.password undefined
    res = await request(app).get(`/api/check-password/${USER2_ID}/some-password`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    // user not found
    res = await request(app).get(`/api/check-password/${testHelper.GetRandromObjectIdAsString()}/some-password`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    // wrong user id
    res = await request(app).get('/api/check-password/0/some-password').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/users/:page/:size', () => {
  it('should get users', async () => {
    ...
    let res = await request(app).post(`/api/users/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBeGreaterThan(3);
    payload.user = '';
    res = await request(app).post(`/api/users/${testHelper.PAGE}/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBeGreaterThan(3);
    res = await request(app).post(`/api/users/unknown/${testHelper.SIZE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/has-password/:id', () => {
  it('should get users', async () => {
    const token = await testHelper.signinAsAdmin();
    // test success
    let res = await request(app).get(`/api/has-password/${USER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    // test success (user not found)
    res = await request(app).get(`/api/has-password/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    // test failure (wrong user id)
    res = await request(app).get('/api/has-password/wrong-id').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-users', () => {
  it('should delete users', async () => {
    const token = await testHelper.signinAsAdmin();
    let payload = [USER1_ID, USER2_ID, ADMIN_ID];
    ...
    let res = await request(app).post('/api/delete-users').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/delete-users').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/delete-users').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/verify-recaptcha/:token/:ip', () => {
  it('should verify reCAPTCHA', async () => {
    ...
    await jest.isolateModulesAsync(async () => {
      const newApp = (await import("../src/app.js")).default;
      const res = await request(newApp).post(`/api/verify-recaptcha/${recaptchaToken}/${ip}`);
      expect(res.statusCode).toBe(200);
    });
...
describe('POST /api/send-email', () => {
  it('should send an email', async () => {
    ...
    let res = await request(app).post('/api/send-email').set('Origin', env.FRONTEND_HOST).send(payload);
    expect(res.statusCode).toBe(200);
    // test success (newsletter form)
    payload.isContactForm = false;
    payload.message = '';
    res = await request(app).post('/api/send-email').set('Origin', env.FRONTEND_HOST).send(payload);
    expect(res.statusCode).toBe(200);
    // test failure (no Origin)
    res = await request(app).post('/api/send-email').send(payload);
    expect(res.statusCode).toBe(400);
    // test failure (Not allowed by CORS)
    res = await request(app).post('/api/send-email').set('Origin', 'https://unknown.com').send(payload);
    expect(res.statusCode).toBe(500);
  });
});