...
import stripeAPI from "../src/payment/stripe.js";
...
afterAll(async () => {
  ...
  await Property.deleteMany({
    _id: {
      $in: [PROPERTY1_ID, PROPERTY2_ID]
    }
  });
  // delete renters
  await User.deleteMany({
    _id: {
      $in: [RENTER1_ID, RENTER2_ID]
    }
  });
...
describe('POST /api/create-booking', () => {
  it('should create a booking', async () => {
    ...
    let res = await request(app).post('/api/create-booking').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    BOOKING_ID = res.body._id;
    res = await request(app).post('/api/create-booking').set(env.X_ACCESS_TOKEN, token);
...
describe('POST /api/checkout', () => {
  it('should checkout', async () => {
    ...
    jest.spyOn(stripeAPI.customers, 'create').mockResolvedValue({
      id: 'cus_test_123'
    });
    jest.spyOn(stripeAPI.customers, 'retrieve').mockResolvedValue({
      id: 'cus_test_123'
    });
    jest.spyOn(stripeAPI.customers, 'del').mockResolvedValue({
      deleted: true
    });
    jest.spyOn(stripeAPI.paymentIntents, 'create').mockResolvedValue({
      id: 'pi_test_123',
      client_secret: 'secret'
    });
    // Este espía es dinámico. Responde primero que no está pagado, y para el resto del test que sí está pagado.
    jest.spyOn(stripeAPI.paymentIntents, 'retrieve').mockResolvedValueOnce({
      id: 'pi_test_123',
    ...
    jest.spyOn(stripeAPI.paymentIntents, 'confirm').mockResolvedValue({
      id: 'pi_test_123',
      status: 'succeeded'
    });
    // ------------------------------------------------
    ...
    let res = await request(app).post('/api/checkout').send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post('/api/checkout').send(payload);
    ...
    const renter2 = await User.findOne({
      email: payload.renter.email
    });
    ...
    res = await request(app).post('/api/checkout').send(payload);
    expect(res.statusCode).toBe(204);
    res = await request(app).post('/api/checkout').send({
      booking: {
        renter: RENTER1_ID
      }
    });
...
describe('POST /api/update-booking', () => {
  it('should update a booking', async () => {
    ...
    let res = await request(app).put('/api/update-booking').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).put('/api/update-booking').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body.property).toBe(PROPERTY2_ID);
    expect(res.body.price).toBe(4800);
    expect(res.body.status).toBe(movininTypes.BookingStatus.Paid);
    ...
    let pushToken = new PushToken({
      user: payload.renter,
      token: 'ExponentPushToken[CokU9KJ9-Yq2ulVTyYOI8J]'
    });
    ...
    res = await request(app).put('/api/update-booking').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(400);
    res = await request(app).put('/api/update-booking').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/update-booking-status', () => {
  it('should update booking status', async () => {
    ...
    let res = await request(app).post('/api/update-booking-status').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    let booking = await Booking.findById(BOOKING_ID);
    expect(booking?.status).toBe(movininTypes.BookingStatus.Reserved);
    res = await request(app).post('/api/update-booking-status').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    booking = await Booking.findById(BOOKING_ID);
    expect(booking?.status).toBe(movininTypes.BookingStatus.Reserved);
    res = await request(app).post('/api/update-booking-status').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/booking/:id/:language', () => {
  it('should get a booking', async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/booking/${BOOKING_ID}/${testHelper.LANGUAGE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    expect(res.body.property._id).toBe(PROPERTY2_ID);
    res = await request(app).get(`/api/booking/${testHelper.GetRandromObjectIdAsString()}/${testHelper.LANGUAGE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).get(`/api/booking/${nanoid()}/${testHelper.LANGUAGE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/booking-id/:sessionId', () => {
  it('should get a booking id from session id', async () => {
    ...
    await booking.save();
    let res = await request(app).get(`/api/booking-id/${sessionId}`);
    expect(res.statusCode).toBe(200);
    expect(res.body).toBe(booking._id.toString());
    // test success (booking not found)
    res = await request(app).get(`/api/booking-id/${nanoid()}`);
    expect(res.statusCode).toBe(204);
...
describe('POST /api/bookings/:page/:size/:language', () => {
  it('should get bookings', async () => {
    ...
    let res = await request(app).post(`/api/bookings/${testHelper.PAGE}/${testHelper.SIZE}/${testHelper.LANGUAGE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    ...
    res = await request(app).post(`/api/bookings/${testHelper.PAGE}/${testHelper.SIZE}/${testHelper.LANGUAGE}`).set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBe(1);
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body[0].resultData.length).toBe(1);
    res = await request(app).post(`/api/bookings/${testHelper.PAGE}/${testHelper.SIZE}/${testHelper.LANGUAGE}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('GET /api/has-bookings/:renter', () => {
  it("should check renter's bookings", async () => {
    const token = await testHelper.signinAsAdmin();
    let res = await request(app).get(`/api/has-bookings/${RENTER1_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    res = await request(app).get(`/api/has-bookings/${AGENCY_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    const booking = await Booking.findById(BOOKING_ID);
    expect(booking?.status).toBe(movininTypes.BookingStatus.Reserved);
    res = await request(app).get(`/api/has-bookings/${nanoid()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/cancel-booking/:id', () => {
  it('should cancel a booking', async () => {
    ...
    let res = await request(app).post(`/api/cancel-booking/${BOOKING_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(200);
    booking = await Booking.findById(BOOKING_ID);
    ...
    await booking.save();
    res = await request(app).post(`/api/cancel-booking/${BOOKING_ID}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    booking.agency = agencyId;
    await booking.save();
    res = await request(app).post(`/api/cancel-booking/${testHelper.GetRandromObjectIdAsString()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(204);
    res = await request(app).post(`/api/cancel-booking/${nanoid()}`).set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('POST /api/delete-bookings', () => {
  it('should delete bookings', async () => {
    ...
    const payload = bookings.map(u => u._id.toString());
    let res = await request(app).post('/api/delete-bookings').set(env.X_ACCESS_TOKEN, token).send(payload);
    expect(res.statusCode).toBe(200);
    bookings = await Booking.find({
      renter: {
        $in: renters
    ...
    res = await request(app).post('/api/delete-bookings').set(env.X_ACCESS_TOKEN, token);
    expect(res.statusCode).toBe(400);
    await testHelper.signout(token);
  });
});
describe('DELETE /api/delete-temp-booking', () => {
  it('should delete temporary booking', async () => {
    ...
    await booking.save();
    let res = await request(app).delete(`/api/delete-temp-booking/${booking._id.toString()}/${sessionId}`);
    expect(res.statusCode).toBe(200);
    ...
    try {
      await databaseHelper.close();
      res = await request(app).delete(`/api/delete-temp-booking/${booking._id.toString()}/${sessionId}`);
      expect(res.statusCode).toBe(400);
    } catch (err) {
      console.error(err);
    } finally {
