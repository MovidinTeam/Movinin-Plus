...
import * as databaseLangHelper from "../src/utils/databaseLangHelper.js";
...
describe('Test database initialization', () => {
  it('should initialize database', async () => {
    let res = await databaseHelper.connect(env.DB_URI, false, false);
    expect(res).toBeTruthy();
    const lv1 = new LocationValue({
      language: 'en',
      value: 'location'
    });
    await lv1.save();
    const lv2 = new LocationValue({
      language: 'es',
      value: 'localización'
    });
    await lv2.save();
    const l1 = new Location({
      ...
      values: [lv1._id.toString(), lv2._id.toString()]
    });
    await l1.save();
    const l2 = new Location({
      country: testHelper.GetRandromObjectIdAsString(),
      values: [lv2._id.toString()]
    });
    await l2.save();
    ...
    const cv1 = new LocationValue({
      language: 'en',
      value: 'country'
    });
    await cv1.save();
    const cv2 = new LocationValue({
      language: 'es',
      value: 'país'
    });
    await cv2.save();
    const c1 = new Country({
      values: [cv1._id.toString(), cv2._id.toString()]
    });
    await c1.save();
    const c2 = new Country({
      values: [cv2._id.toString()]
    });
    await c2.save();
    ...
    await LocationValue.deleteMany({
      _id: {
        $in: [...location1.values, ...location2.values]
      }
    });
    ...
    await LocationValue.deleteMany({
      _id: {
        $in: [...country1.values, ...country2.values]
      }
    });
...
describe('checkAndUpdateTTL', () => {
  it('logs error when dropIndex throws (non IndexNotFound)', async () => {
    ...
    await jest.isolateModulesAsync(async () => {
      const {
        checkAndUpdateTTL
      } = await import("../src/utils/databaseHelper.js");
      const model = {
        modelName: 'TestModel',
        collection: {
          indexes: jest.fn(() => Promise.resolve([{
            name,
            expireAfterSeconds: seconds - 1
          } // force TTL mismatch
          ])),
          dropIndex: jest.fn(() => Promise.reject(error))
        }
      };
      await expect(checkAndUpdateTTL(model, name, seconds)).rejects.toThrow(error);
      expect(model.collection.dropIndex).toHaveBeenCalledWith(name);
      expect(logger.error).toHaveBeenCalledWith(`Failed to drop TTL index "TestModel.${name}":`, error);
